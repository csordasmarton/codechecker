<template>
  ...
  >
    ...
    >
      <template v-slot:activator="{ on }">
        ...
        >
          ...
          <div class="git-time" :title="commit.committed_datetime">
            on {{ date }}
          </div>
        </component>
      </template>
    ...
    <span
      ...
      :style="{'background-color': commitColor }"
    >
      {{ number }}
    </span>
  </div>
...
export default {
  ...
  computed: {
    ...
    commitColor() {
      return "rgb(181, 202, 128)";
    },
    date() {
      return this.commit.committed_datetime.split(" ")[0];
    },
    message() {
